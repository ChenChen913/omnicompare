...
- **纯 HTML 项目没有播放语义**：当前项目无任何视频时，「同时播放 / 暂停」按钮整组隐藏，「显示」菜单中循环/静音/速度三项禁用并附提示（2026-08-29 用户确认「没有播放按钮这一说」）
...
1. **多项目管理**：顶栏项目切换器可新建/切换项目（按进行中/草稿/归档分组）；侧栏项目卡「管理」支持改名、切换状态、删除项目（默认项目不可删）；当前项目记住在本地，刷新后保持
2. **选数量与矩阵**：点右上「布局」→ 上半区选视频个数（1–12），下半区选几行几列；标注「补」的矩阵不整除，末尾会留空格子。单视频（1 个）自动满幅
3. **上传**：点任意空位选文件；或把文件直接拖进页面；或点「一键导入」多选文件。文件多于空位时自动扩位
4. **写标题**：每个视频下方的标题框，点击输入，失焦后自动收折，600ms 防抖保存，最多 100 字
5. **播放与显示**：有视频时点「同时播放」一起看、「暂停」全停；循环/静音/播放速度/标题显隐/属性显隐统一收在顶栏「显示」菜单，状态保存在服务端随项目记住；纯 HTML 项目自动隐藏播放按钮（页面打开即自动运行）
6. **删除**：卡片右下角垃圾桶删除单个；导航栏垃圾桶清空全部（需确认，不可恢复）
7. **缩减位数**：若被移除区间里有视频，会弹确认框告知删除数量，确认后视频文件同步删除
