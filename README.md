...
5. **对比播放**：含视频的项目点「同时播放」会把所有视频归零后一起起播；循环、静音、倍速与背景音乐收在顶栏「播放」菜单，标题与属性显隐收在「标题」菜单，状态保存在服务端。要配乐时点顶栏「音乐」按钮上传一首（≤50MB），音量在「播放 → 背景音乐」里调。专注模式下按空格或 F 即可一键播放 / 暂停（视频与背景音乐同步），配合隐式控件可全程键盘录屏；重新备场用「播放」菜单的「全部归零并暂停 / 全部归零并播放」，矩阵下方可按需显示提示词与署名（「标题」菜单开关）。
