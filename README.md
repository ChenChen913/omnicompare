...
- **批量播放控制**：同时播放 / 暂停 / 循环 / 全局静音 / 倍速。纯 HTML 项目没有播放语义，主动作自动变为「刷新全部」。专注模式下空格 / F 键一键切换：视频与背景音乐从头同步起播 ⇄ 全部暂停，录屏全程无需鼠标入镜。
...
5. **对比播放**：含视频的项目点「同时播放」会把所有视频归零后一起起播；循环、静音、倍速与背景音乐收在顶栏「播放」菜单，标题与属性显隐收在「标题」菜单，状态保存在服务端。要配乐时点顶栏「音乐」按钮上传一首（≤50MB），音量在「播放 → 背景音乐」里调。专注模式下按空格或 F 即可一键播放 / 暂停（视频与背景音乐同步），配合隐式控件可全程键盘录屏。
