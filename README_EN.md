...
- **Multi-project management**: create multiple independent projects (content, layout, and settings fully isolated); the top bar groups and switches them by status (active / draft / archived); the sidebar supports renaming, status changes, and project deletion (the default project is protected).
...
- **Batch playback control**: play all / pause all / restart all / loop / global mute / playback speed (0.5×–2×), applied to all video items; loop, mute, speed, title and info visibility live together in the top-bar "Display" menu; playback controls hide automatically when a project has no videos (pure-HTML projects have no playback semantics), and HTML pages keep their own runtime rhythm and are never timeline-controlled.
- **Aspect ratio and title/info visibility**: switch the global card-frame ratio (original / 16:9 / 9:16 / 1:1) at any time, with per-card overrides; content is always shown fully via contain (never cropped); the title box and the info row below it (filename/size/actions) can be hidden independently with one click each, for pure viewing and screen recording. All settings live on the server and sync across devices.
...
- **Refined interactions**: confirmation before shrinking slots, automatic cleanup of replaced files, non-stacking toasts, zero layout jitter when switching layouts, and smooth project switching that keeps the old content visible until new data arrives (no skeleton flicker); the sidebar collapse control is a slim handle attached to the sidebar's right edge instead of occupying the top bar.
