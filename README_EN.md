...
- **Batch playback**: play-all / pause / loop / global mute / playback rate. Pure-HTML projects have no playback semantics, so the primary action becomes "Refresh all". In Focus mode, Space / F toggles one-key playback (videos and BGM rewind together and start in sync) ⇄ pause-all — no mouse on screen while recording. The "Play" menu also offers "Reset all (paused)" / "Reset all (playing)": videos rewind to 0, pages reload, BGM rewinds (R key = reset paused in Focus mode).
...
- **Prompt & byline**: show a prompt box and a byline row (reviewer / @handle / #tags) below the matrix; click to edit, auto-save on blur, stored per project. Toggles live in the "Title" menu — bring them into frame while recording.
- **Anti-theft watermark**: every setting lives in the top-bar "Watermark" button — text, font size, font family, color depth (follow theme / white / black), opacity, bold, and drift speed (slow / normal / fast). The mask layer tracks the wall's actual bounds: with one video the watermark roams that single card, with many it roams the whole wall; the watermark glides above the videos in a screensaver-like loop to deter re-uploads. Stored per project.
