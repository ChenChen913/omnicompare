...
- **Multi-project management**: create multiple independent projects (content, layout, and settings fully isolated); the top bar groups and switches them by status (active / draft / archived) with badges showing each project's video / page counts; the sidebar supports renaming, status changes, and project deletion (the default project is protected).
...
- **Top bar adapts to content automatically**: projects containing videos get "Play all / Pause all" plus loop / mute / speed (0.5×–2×); pure-HTML projects have no playback semantics — the primary action becomes "Refresh all" (reloads every page back to its initial state); in mixed projects playback controls only affect videos. Loop, mute, speed, title and info visibility live together in the top-bar "Display" menu. Import whatever you like — the top bar grows exactly the controls that fit, with no mode to pick beforehand.
