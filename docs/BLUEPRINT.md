...
| D3 | HTML 范围 | MVP 只收**单文件自包含 .html**（内联 CSS/JS）；zip 资源包已于二期 Step B 落地（须含根级 index.html，资产扩展名白名单，zip-slip 严防） |
| D4 | 图片类型 | 原定第二阶段，已于二期 Step A 落地（kind=image，PNG/JPG/GIF/WebP/SVG/BMP/AVIF，≤20MB；SVG 以 <img> 渲染 + CSP 沙箱双保险，内嵌脚本不执行） |
...
| D10 | 顶栏信息架构 | 顶栏控制分组排列（播放组 / 布局·显示·导入组 / 模式组，竖线分隔）：循环/静音/速度/标题/属性收进「显示」下拉；侧栏开关改为贴在侧栏右缘的长条把手（不占顶栏）；主题切换为顶栏最右上角无框太阳/月亮图标；纯 HTML 项目（无视频）无播放语义，播放控件整组隐藏、显示菜单中视频类选项禁用；项目切换保留旧内容渐隐过渡、不用骨架屏防跳动；底部注意事项分行展示、关键信息加粗（2026-08-29 用户反馈） |
| D11 | 顶栏随内容自动适配 + 恒定高度 | 顶栏形态由项目内容**全自动派生**、无需用户选择模式：含视频 → 「同时播放 / 暂停」；纯 HTML → 主动作变为「刷新全部」（重载全部 iframe 回到初始状态），显示菜单中循环/静音/速度禁用并提示；混合项目播放组与刷新组并存语义按类型生效；空项目仅布局/导入。项目切换器条目以角标（🎬视频数 / `</>`网页数）预告形态，侧栏项目卡显示「x 视频 · y 网页」构成。结构上顶栏**固定两行**（第一行品牌+项目切换+使用须知书本按钮+主题；第二行功能按钮不换行、窄屏横向滑动），行高不随按钮显隐变化——修复切换项目时 flex-wrap 行数 1↔2 跳变把主体内容推上推下的「一缩一缩」震动（1024px 视口实测 header 65↔115px，修复后双向往返恒 113px）。底部常驻注意事项移除，改为项目切换器右侧书本按钮弹出「使用须知」Dialog（点空白/Esc/关闭按钮均可关）（2026-08-29 用户反馈） |
| D12 | 弹层零位移 + 侧栏把手居中 | 根治第二类「一缩一缩」（横向）：Radix modal 弹层（DropdownMenu/Dialog/AlertDialog）打开时 react-remove-scroll-bar 会给 `body[data-scroll-locked]` 注入 `margin-right: 滚动条宽度 !important` 的间隙补偿，而本项目 html 强制常驻滚动条（`overflow-y: scroll + scrollbar-gutter: stable`），滚动条从不消失，补偿纯有害——每次弹层开/关整个 body 被压窄/弹回 15~17px（弹层测量与注入机制详见 globals.css 注释）；无头测试环境是 overlay 滚动条（gap 恒 0）故此前自动化无法复现，验证脚本用 `defineProperty` 桩化 `window.innerWidth` 模拟经典滚动条（scripts/test-shrink.sh）。修复：`html body[data-scroll-locked]{margin-right:0!important;padding-right:0!important}`（特异性 0-1-2 压过注入的 0-1-1），弹层滚动锁定功能不受影响（实测开合 bodyMR 恒 0、headerH 恒 113、主题按钮与主内容零位移）。侧栏收缩把手从 `mt-20` 偏上位置改为**垂直居中于侧栏可视区**：sticky 容器定高 `calc(100dvh-146px)`（顶栏下沿 130px + 底部留白 16px）+ 把手 `self-center`，滚动/收起/展开时把手位置恒定（实测 900 视口下 top 恒 475、滚动 270px 后仍钉在原地）（2026-08-29 用户反馈） |
| D13 | 设置入口评估（二期 Step E） | 侧栏「设置」入口**维持占位禁用**：全部设置项已收编于顶栏「布局」（比例/矩阵/数量）与「显示」（循环/静音/速度/标题/属性/模糊填充）两个弹层，独立设置页属重复入口；侧栏入口的 title 提示「设置已收编于顶栏」。待出现真正的全局设置（如外链 CDN 开关、鉴权配置）再启用（2026-08-30 评估） |
...
| 9 | 打磨与文档：README/PROJECT 更新、对抗测试脚本扩展 HTML/排序用例 | 完成（UI 打磨随 D10/D11/D12 落地；对抗脚本新增 §18 HTML 专项（超大文件/原位替换清理/v2 一致性/跨项目解析与项目删除清理）与 §19 排序专项（排列校验对抗/标题跟随/紧凑序/文件不变量），110→137 项全绿；生产构建全链路验证（17 路由 + standalone 冒烟 + tsc/eslint）；Docker 部署三件套（Dockerfile/docker-compose/.dockerignore，多阶段构建 + 数据卷 + 健康检查）+ 备份脚本 backup-data.sh；CI（GitHub Actions：install/lint/tsc/build）；PROJECT.md 新增 Docker 部署与备份章节。事故记录：沙箱重置暴露两个 upload 路由从未入 git，已从历史提交恢复并纳入跟踪，教训固化至 LESSONS-LEARNED §5.3） |
| 10 | 第二阶段：图片类型 / zip 资源包 / 模糊背景填充 / 项目库视图 / 设置入口评估 | 完成（Step A：kind=image 全链路（上传/流式服务/<img> 渲染/SVG CSP 沙箱/清理不变量/顶栏角标三类型）；Step B：zip 页面包（fflate 解压 + zip-slip 路径归一化严防 + 资产白名单 + 三重限额 + 新增 /api/bundles 子路径沙箱服务 + 包目录随清单清理）；Step C：letterboxFill 设置（视频同步镜像副本铺底/图片静态模糊层/HTML 豁免，显示菜单开关）；Step D：侧栏「库」激活为项目库视图（项目卡片网格/点击切换/状态与构成徽标/两行高度恒定保持）；Step E（D13）：设置入口维持占位。对抗 137→160 项（新增 §20 图片/§21 zip 包专项）+ 冒烟 38 项全绿；浏览器端到端验证通过；prisma 脚手架残留终清（依赖/目录/db:* 脚本移除）） |
